	...
	public void receiveFileWithAckLost(float loss) throws IOException {
		...
		while (currentTotal < totalBytes) {
			...
			if(x == dataSeg.getChecksum()){
				...
				if (isLost(loss)){
					System.out.println("SERVER: The ACK was *lost* in transit");
					System.out.println("*************************** ");

					/* write the payload of the data segment to output file so simulate just the acknoledgement being lost*/
					myWriter.write(dataSeg.getPayLoad());
					currentTotal = currentTotal + dataSeg.getSize();
					continue;
				}
